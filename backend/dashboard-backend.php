<?php
    // Fetch student data
    $sql = "SELECT ID, NAME, GENDER, EMAIL FROM new_admission ORDER BY ID DESC LIMIT 3";
    ...
    if ($result->num_rows > 0) {
        ...
        echo "<div class='table-row' style='grid-template-columns: 100%;font-size:1.5rem;margin-top:1rem;pointer-events:none;'><div class='table-col'>No Data found</div></div>";
    }

    // Fetch teacher data
    $sql = "SELECT ID, NAME, GENDER, EMAIL FROM new_staff ORDER BY ID DESC LIMIT 2";
    $result = $conn->query($sql);
